...
class DependencySorter
{
    ...
    /**
     * Array of dependent instances associated with the items.
     *
     * @var array
     */
     ...
     *
     * @param \Cartalyst\Dependencies\DependentInterface|string $item
     * @param array|string                                      $dependencies
     *
     * @return void
     */
    public function add($item, $dependencies = []): void
    {
        if ($item instanceof DependentInterface) {
            $this->addDependent($item);
        } else {
    ...
    /**
     * Sorts the items and returns an array of items in order of their
     * dependencies, where the first item has the least dependencies
     * and can be used first and the last item can't be used until
     * all the other items have been properly resolved.
     *
     * @return array
     */
     ...
     * @return void
     */
    protected function addItem(string $item, $dependencies = []): void
    {
     ...
     * @return void
     */
    protected function addDependent(DependentInterface $dependent): void
    {
     ...
     * @return void
     */
    protected function evaluateItem(string $item, array $dependencies, array $original, array &$sorted, &$items): void
    {
     ...
     * @return bool
     */
    protected function dependencyIsValid(string $item, string $dependency, array $original, array $items): bool
    {
        ...
        if (isset($items[$dependency]) and in_array($item, $items[$dependency])) {
     ...
     * @return array
     */
    protected function retrieve(array $sorted): array
    {
        $retrieved = [];
